...
const HomeMenu = () => {
  ...
  // Utilisez le hook useAddNewMenuMutation pour ajouter un nouveau menu
  const [addNewMenu] = useAddNewMenuMutation();

  // Utilisez le hook useDeleteMenuMutation pour supprimer un menu existant
  ...
  const handleValideMenu = async () => {
    ...
    for (let x = 0; x < listeMenus.length && x < 9000; x++) {
      ...
      if (menuliste) {
        ...
        // }
        try {
          // Utilisez le hook deleteMenu pour supprimer le menu
          await deleteMenu({ id: menuliste.id });
          console.log("DELETE MENU BDD");
          // Vous n'avez plus besoin de dispatch(deleteListeMenu(menuliste._id));
          break;
        } catch (error) {
          console.error(
            "Une erreur s'est produite lors de la suppression du menu :",
    ...
    // Enregistrement du nouveau menu en utilisant le hook addNewMenu
    try {
      await addNewMenu({
        prefNbJ: prefSelected[0],
        prefNbMeal: prefSelected[1],
      ...
      console.error(
        "Une erreur s'est produite lors de l'ajout du menu :",
        error
      );
    }
